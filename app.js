...
function frontSvg(f,b,l,x,y,w,h,p){
 if(f.shape==='none')return'';
 let cab=Math.max(72, p.cab/b.len*w);
 const makeCab=(side)=>{
   const left=side==='L'; const sx=left?x:x+w-cab; const sign=left?1:-1;
   const nose=left?`M${x} ${y+13} Q${x+20} ${y} ${x+cab} ${y}`:`M${x+w-cab} ${y} Q${x+w-20} ${y} ${x+w} ${y+13}`;
   const body=left?`${nose} L${x+cab} ${y+h} L${x} ${y+h} Z`:`${nose} L${x+w} ${y+h} L${x+w-cab} ${y+h} Z`;
   const wx=left?x+28:x+w-84; const doorX=left?x+cab-46:x+w-cab+9; const lightX=left?x+cab-20:x+w-cab+20;
   return `<g class="front-${side}">
    <path d="${body}" fill="url(#metal)" stroke="#07111f" stroke-width="4"/>
    <path d="M${left?x+8:x+w-8} ${y+22} Q${left?x+30:x+w-30} ${y+7} ${left?x+72:x+w-72} ${y+5}" stroke="#f8fafc" stroke-width="3" opacity=".85" fill="none"/>
    <rect x="${wx}" y="${y+33}" width="56" height="42" rx="9" fill="url(#glass)" stroke="#0f172a" stroke-width="2"/>
    <rect x="${doorX}" y="${y+20}" width="34" height="92" rx="4" fill="#e2e8f0" stroke="#172235" stroke-width="2" opacity=".95"/>
    <rect x="${doorX+6}" y="${y+33}" width="22" height="31" rx="4" fill="url(#glass)"/>
    <circle cx="${lightX}" cy="${y+90}" r="4" fill="#fbbf24"/>
    <rect x="${left?x+18:x+w-88}" y="${y+h-29}" width="70" height="13" rx="3" fill="#cbd5e1" opacity=".76"/>
   </g>`
 };
 if(['e259','series787'].includes(f.shape)){
   return `<g>${makeCab('L')}${makeCab('R')}<path d="M${x+20} ${y+h-10} Q${x+w/2} ${y+h+18} ${x+w-20} ${y+h-10}" stroke="#64748b" fill="none" opacity=".55"/></g>`;
 }
 return makeCab('L')+makeCab('R');
}
...
function cooler(x,y,w){
 let slats=Array.from({length:12},(_,k)=>`<line x1="${x+18+k*(w-36)/11}" x2="${x+18+k*(w-36)/11}" y1="${y+6}" y2="${y+25}" stroke="#475569" opacity=".45"/>`).join('');
 return `<g class="cooler" filter="url(#shadow)">
  <rect x="${x}" y="${y}" width="${w}" height="34" rx="5" fill="#d8e2ec" stroke="#64748b"/>
  <rect x="${x+10}" y="${y+5}" width="${w-20}" height="24" rx="2" fill="#aebccd" opacity=".65"/>
  ${slats}
  <circle cx="${x+w*.34}" cy="${y+17}" r="9" fill="#94a3b8" opacity=".55"/>
  <circle cx="${x+w*.66}" cy="${y+17}" r="9" fill="#94a3b8" opacity=".55"/>
  <rect x="${x+4}" y="${y+2}" width="${w-8}" height="4" fill="#f8fafc" opacity=".45"/>
 </g>`
}
...
function panto(px,y,dir){
 const lean=dir>=0?1:-1;
 return `<g class="panto" stroke-linecap="round" stroke-linejoin="round">
  <rect x="${px-58}" y="${y+2}" width="116" height="8" rx="3" fill="#94a3b8" stroke="#64748b"/>
  <rect x="${px-42}" y="${y-5}" width="84" height="7" rx="3" fill="#cbd5e1" stroke="#64748b"/>
  <circle cx="${px-36}" cy="${y+2}" r="7" fill="#dbeafe" stroke="#94a3b8"/>
  <circle cx="${px+36}" cy="${y+2}" r="7" fill="#dbeafe" stroke="#94a3b8"/>
  <path d="M${px-40} ${y-4} L${px} ${y-58} L${px+54} ${y-14}" fill="none" stroke="#e5edf7" stroke-width="5"/>
  <path d="M${px+38} ${y-4} L${px} ${y-58} L${px-46} ${y-13}" fill="none" stroke="#cbd5e1" stroke-width="4" opacity=".9"/>
  <path d="M${px-24} ${y-7} L${px} ${y-32} L${px+27} ${y-8}" fill="none" stroke="#e2e8f0" stroke-width="3" opacity=".95"/>
  <path d="M${px-12} ${y-34} L${px+12} ${y-34}" stroke="#f8fafc" stroke-width="3" opacity=".85"/>
  <line x1="${px}" y1="${y-58}" x2="${px+lean*86}" y2="${y-76}" stroke="#dbe7f2" stroke-width="5"/>
  <line x1="${px+lean*86}" y1="${y-76}" x2="${px+lean*117}" y2="${y-80}" stroke="#dbe7f2" stroke-width="4"/>
  <path d="M${px+lean*113} ${y-82} q${lean*14} 6 0 12" stroke="#dbe7f2" stroke-width="3" fill="none"/>
  <path d="M${px-54} ${y+15} H${px+54}" stroke="#7c8da3" stroke-width="2"/>
 </g>`
}
...
function saveRvs(){const blob=new Blob([JSON.stringify({rvs:'1.3',state},null,2)],{type:'application/json'});let a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='odorail-beta13.rvs';a.click();}
