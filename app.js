...
svg+=`<rect x="55" y="112" width="70" height="55" rx="6" fill="url(#glass)"/><rect x="875" y="112" width="70" height="55" rx="6" fill="url(#glass)"/>`;
if(f.id.includes('A771')){svg+=`<rect x="65" y="120" width="105" height="45" rx="9" fill="#111827"/><rect x="74" y="128" width="36" height="28" rx="4" fill="#334155"/><rect x="118" y="128" width="42" height="28" rx="4" fill="#334155"/><circle cx="72" cy="184" r="5" fill="#fef3c7"/><circle cx="155" cy="184" r="5" fill="#fef3c7"/>`;}
...
function saveRvs(){const data={app:'RVS',version:'β1.5',state};const blob=new Blob([JSON.stringify(data,null,2)],{type:'application/json'});const a=document.createElement('a');a.href=URL.createObjectURL(blob);a.download='odorail_beta15.rvs';a.click()}
...
function exportPng(){const svg=document.getElementById('trainSvg');const data=new XMLSerializer().serializeToString(svg);const img=new Image();const blob=new Blob([data],{type:'image/svg+xml'});const url=URL.createObjectURL(blob);img.onload=()=>{const c=document.createElement('canvas');c.width=1600;c.height=576;const ctx=c.getContext('2d');ctx.fillStyle='#e8eef8';ctx.fillRect(0,0,c.width,c.height);ctx.drawImage(img,0,0,c.width,c.height);const a=document.createElement('a');a.href=c.toDataURL('image/png');a.download='rvs_beta15.png';a.click();URL.revokeObjectURL(url)};img.src=url}
init();
